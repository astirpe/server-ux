...
class TestTierValidationDelegation(CommonTierValidation):
    ...
    def setUpClass(cls):
        ...
        cls.admin_user.write({"groups_id": [(4, cls.delegation_admin_group.id)]})

        cls.test_group = cls.env["res.groups"].create({"name": "Test Review Group"})
        cls.test_user_1.write({"groups_id": [(4, cls.test_group.id)]})
        cls.test_user_2.write({"groups_id": [(4, cls.test_group.id)]})
    ...
    def test_06_user_returns_from_holiday_default(self):
        ...
        review_while_away.invalidate_recordset(["reviewer_ids"])
        self.assertIn(
            self.user_delegator,
    ...
    def test_17_delegation_by_field_reviewer(self):
        ...
        """
        self.env.user.clear_caches()  # Ensure fresh reads

        # 1. Setup: Create a tier definition that uses a field to find the reviewer.
        ...
        # 2. Action: The user goes on holiday. This triggers the fixed `write`
        # method, which in turn calls the fixed `_recompute_reviews_for_users`.
        self.user_delegator.write(
            {"on_holiday": True, "validation_replacer_id": self.user_replacer_b.id}
        )

        # 3. Assert: The pending review should now be assigned to the replacer.
        review.invalidate_recordset(["reviewer_ids"])  # Refresh the field from the DB
        self.assertIn(
            self.user_replacer_b,
            review.reviewer_ids,
    ...
    def test_18_change_replacer_while_on_holiday(self):
        ...
        """
        # 1. Setup: User is on holiday, and a review is delegated to Replacer B.
        self.user_delegator.write(
            {"on_holiday": True, "validation_replacer_id": self.user_replacer_b.id}
        )
        ...
        # 2. Action: While still on holiday, the user changes their replacer to C.
        ...
        # 3. Assert: The review should be moved from B to C.
        review.invalidate_recordset(["reviewer_ids"])
        self.assertIn(
            self.user_replacer_c,
            review.reviewer_ids,
